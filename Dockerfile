...
RUN ls && mkdir -p /opt/italian-nlp-library/target/test-classes/ && cp -v it_verb_model.db /opt/italian-nlp-library/target/test-classes/
RUN wget https://github.com/jacopofar/italian-nlp-library/releases/download/v0.1/it_POS_model.db
RUN cp -v it_POS_model.db /opt/italian-nlp-library/target/test-classes/
RUN wget https://github.com/jacopofar/italian-nlp-library/releases/download/v0.1/it-token.bin
RUN ls && mkdir -p target/test-classes && cp -v it-token.bin /opt/italian-nlp-library/target/test-classes/
RUN wget https://github.com/jacopofar/italian-nlp-library/releases/download/v0.1/it-sent.bin
RUN cp -v it-sent.bin /opt/italian-nlp-library/target/test-classes/
RUN wget https://github.com/jacopofar/italian-nlp-library/releases/download/v0.1/it-pos-maxent.bin
RUN cp -v it-pos-maxent.bin /opt/italian-nlp-library/target/test-classes/
RUN wget https://github.com/jacopofar/italian-nlp-library/releases/download/v0.1/stopwords.txt
RUN cp -v stopwords.txt /opt/italian-nlp-library/target/test-classes/
